...
# ╔═╡ eddb6b07-407a-45e4-ad56-6462ae456197
using Plots
...
> The waves in the Kelvin wake scale with $\ell$ but span a range of sizes. The transverse wave has $\lambda=2\pi\ell$ while the main divergent wave has $\lambda=\frac{4\pi}{3}\ell$, but smaller divergent waves can have $\lambda\le\ell$.
...
Not good!
 ...
 - The scaling is terrible. Going from $\ell = 1/4 \rightarrow \ell = 1/8$ requires $4\times N$, $16\times$ the memory, and $22\times$ the time!
...
We developed most of the tools required to set-up and solve a free surface source panel method in this notebook:
...
The only missing piece is writing a FSBC function, which we do in the appendix below to generate the following free surface elevation generated by the submerged sphere."""
...
The example below shows that GMRES is able to converge to a solution using this scheme, although it is much slower (taking O(100) iterations) and misses the fine details in the Kelvin wake.
"""

# ╔═╡ 7ec4f317-5bee-4240-ab4f-b37705257dfc
plt = begin
	# half-sphere
	ℓ = 1/4
	S(θ₁,θ₂) = 0.5SA[cos(θ₁),-sin(θ₂)*sin(θ₁),cos(θ₂)*sin(θ₁)-1.5]
	body = panelize(S,0,π,0,π,hᵤ=ℓ)
	# half-plane
	ζ₀(x,y) = SA[x,y,0]
	hfs = ℓ/3
	x,y = 1:-hfs:-4, hfs/2:hfs:3
	freesurf = measure.(ζ₀,x,y',hfs,hfs,flip=true)
	# pipeline with y-symmetry
	z = FSPanelSystem(body,freesurf;ℓ,sym_axes=2) |> gmressolve! |> ζ
	surface(x,y, ℓ .* z',zlims=(-1,1))
end

# ╔═╡ 9db9947c-9e8e-48b9-81e7-9b981cf849d3
plt
...
manifest_format = "2.0"
project_hash = "6bc89f19bcaef3bdca780252bd6d4ca98b614690"
