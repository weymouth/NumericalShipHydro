...
> - Each constant-strength source panels on the body $\cal B$ is defined as
...
### Activity
 - How is $\ell$ related to the dimensionless Froude-number?
...
1. Free Surface Panels
   ...
   - Solve for $q$ which simultaneously satifies the linear FSBC on ${\cal S}_0$ and the Neumann condition on $\cal B$.
...
2. Neumann-Kelvin Panels
   - Formulte a new Kelvin Green's function $K$ which satifies the linear FSBC by construction. 
...
Free surface panel methods seem conceptually easier, but there is an issue: The panels size must be less than $h<\ell/3$ to resolve the waves and $\ell\ll L$ for surface ships.
...
function fake_FSPanelSystem(ℓ)
	# submerged spherical body with diameter=1
	S(θ₁,θ₂) = 0.5SA[0.5cos(θ₁),-sin(θ₂)*sin(θ₁),cos(θ₂)*sin(θ₁)-2]
	body = panelize(S,0,π,0,2π,hᵤ=ℓ) # these panels can be a bit bigger

	# free surface
	P₀(u,v) = SA[u,v,0]
	freesurf = panelize(P₀,-2,1,-1,1,hᵤ=ℓ/3,flip=true,N_max=3500)

	# concatenate the two arrays and make a BodyPanelSystem
	@show ℓ,length(body),length(freesurf)
	return BodyPanelSystem([body;freesurf])
end

# ╔═╡ 8c218be2-eaae-4135-9de7-98734314e26b
for ℓ in logrange(1,1/8,4)
	# solve, and measure cₚ everywhere
	@time cₚ(directsolve!(fake_FSPanelSystem(ℓ),verbose=false))
end

# ╔═╡ 95f68555-57c6-4152-8722-f2dd057e47a2
md"""
Not good!
 - `ℓ=1/8` is already using N=3620 panels, and requires 270MiB. 
 - The scaling is terrible. Going from $\ell=1/4\rightarrow\ell=1/8$ requires 4x$N$, 10x the memory and 22x the time! 

And $ℓ=1/8$ corresponds to a pretty big Froude number, Fn=$(round(1/√8,digits=2)). Getting to Fn=0.25 takes 30 seconds on my _fancy_ laptop and beyond this gives me an out of memory error!

## Accelerated panel arrays

We will steal ideas from computer science and cosmology to speed up the evaluation of $\Phi(x)$ when $N>O(100)$. 
1. [Bounded Volume Hierarchy] (https://en.wikipedia.org/wiki/Bounding_volume_hierarchy): 

![](https://raw.githubusercontent.com/StellaOrg/ImplicitBVH.jl/refs/heads/main/docs/src/assets/bunny.png)

This ia data structure which is used to rapidly measure properties of descrete geometries, like STLs. Conceptually: We find the bounding box for the whole geometry and then split the box in half. Each half has around N/2 panels, and you can draw the two new boxes around each. This process is repeated recursively until each box has only one panel, which only takes $\log_2(N)$ divisions. 
"""

# ╔═╡ cabdf3d4-9d90-4556-a574-a463586d601f
log2(3620)

# ╔═╡ dd3fd66f-1e26-44b9-9e28-928828b65dd4
md"""
So our biggest example above would only need 12 "levels" of nested boxes. 

It is **very** efficient to find which panels in the BVH are far away from a query point. Say a point $\vec x$ is outside a bounding box on level 4 - then it's also outside all the bounding boxes and the panels within that box. You don't need to check any further, skipping O(N) panels!

2. [Barnes-Hut](https://en.wikipedia.org/wiki/Barnes%E2%80%93Hut_simulation):
...
Galaxy simulations like the one above use a similar "tree" structure, lumping together the influence of groups of stars that are far away from the point $\vec x$. This reduces the cost of the simulation from $O(N^2)$ to order $N\log(N)$. The accuracy vs speed trade off is controlled 
"""

# ╔═╡ 9079b996-f1b5-4506-a57a-fc4ef0708342
md"""## Free surface boundary condition derivation
...
The governing equation is therefore $\frac{\text{d}}{\text{d} t}\cal{S} = 0$. Assuming the surface is steady and doesn't overturn we have $S(\vec x,t) = ζ(x,y)-z$. Also assuming $\vec U = [-U,0,0]$ we can write this kinematic equation as
...
Unfortunately this equation is **nonlinear** (derivatives of the two unknowns $\Phi,\ \zeta$ are multiplied) and applied on the unknown wave elevation $\zeta$. To simplify matters, we **linearize** this equation by further assuming *the amplitude of the disturbance is small* $|\nabla\Phi|\ll U$ and $|\zeta|\ll\ell$ where $\ell$ is the length-scale of the disturbance waves. In this case we have:

$\frac{\partial\Phi}{\partial z} = -U\frac{\partial\zeta}{\partial x} \quad\text{on}\quad z=0$

2. The dynamic condition: $p(\cal{S}) = p_{atm}$, the pressure on $\cal S$ is constant.

Using Bernoulli and the same assumptions as above gives another **nonlinear** equation
...
> Critically, **any** superposition of sinwaves of any amplitude, phase, and direction satisfies this equation. The only restriction is that the projection of the wavelength in x is $\lambda_x=2\pi\ell$.
...
manifest_format = "2.0"
project_hash = "cc53c6108b563b3174e85d553c6aac285e9b4f60"
