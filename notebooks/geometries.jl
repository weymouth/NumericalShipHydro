...
md"""
In this notebook, we're going to practise this first step a few times, setting up a few different geometries.

## New parametric surface
...
Before looking at the code below, think about how **you** would do this!
 1. Given a point $\vec x$, how would you reflect it across to the other side?
 2. Now, what makes up a single `panel`, and how should it be reflected?
 3. Now, how would you reflect all the panels and join them with the original?
...
# 1. reflect a point across an axis
...
double_body = vcat(hull,reflect.(hull,flip=SA[1,1,-1])); #z-reflection

# ╔═╡ af312e79-0e9c-48b3-9f10-3f5f5a51243e
plot_area(double_body,zlims=(-0.3,0.3),aspect_ratio=1,widen=false)
...
# ╔═╡ b2e05301-d354-4bb2-848e-7e513604ce95
begin # double-hull solution
	A_big = ∂ₙϕ.(double_body,double_body',ϕ=∫G)
	b_big = first.(double_body.n)
	q_big = A_big\b_big
end;

# ╔═╡ 5f95737f-5c2c-4be2-b49a-55df5632c4cd
begin # use the demihull
	A = ∂ₙϕ.(demihull,demihull',ϕ=∫G)
	b = first.(demihull.n)
	q = A\b
end;
...
q_big[1:end÷4] ≈ q ? "it matches!! 🥳" : "it doesn't match... 🤢" 
...
So we're left with the annoying situation that it is impossible to get the correct potential flow solution without taking 16× longer using the twice-reflected geometry. 

**Or is it...?**

### Activity
 - Discuss **why** the solutions are incorrect (what's missing?) and how you could change the inputs to `∂ₙϕ` and `added_mass` to get the correct solutions using the `demihull` panels.
 - Write a couple functions to do this and test them out!
