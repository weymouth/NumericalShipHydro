...
begin
	...
	Ni(x,y,z,T) = imag(expintx((z*sqrt(1-T^2)+y*T+im*abs(x))*sqrt(1-T^2)+im*eps()))
...
The plot above is the steady potential and wave field generated by a source moving in the positive x direction under the water. The steady potential at point $\vec \xi$ due to a unit-strength source at point $\vec a$ is the sum of three contributions:

 - The underwater source and it's *image sink* located above the free surface at $\vec a'=[a_x,a_y,-a_z]$
 - A near-field disturbance $N$
 - A wave-like disturbance $W$

$\varphi(\xi) = -\frac 1{|\xi-a|}+\frac 1{|\xi-a'|}+\left[N(\vec x)+W(\vec x)\right]\frac g{U^2}$

where $\vec x = (\vec\xi-\vec a')\frac g{U^2}$ is the Froude-scaled vector from the image sink. The near-field and wave-like contributions are defined by a pair of integrals:
...
where $i$ is the imaginary unit, $\Im(\zeta)$ returns the imaginary part of the complex $\zeta$ and $\text{expintx}(\zeta)=e^\zeta E_1(\zeta)$ is the scaled [exponential integral](https://en.wikipedia.org/wiki/Exponential_integral).
...
The plots show the integrands depend sensitively on $\vec x$, especially $W_i$ which oscillates extremely fast when $x^2+y^2 \gg z^2$. Our previous method of a 2-point (or even a 16-point) Gauss-Legendre quadrature won't be able to accurately estimate the integrals!
...
md"
Overall we can see:
 - The function does **all** the work for us and the error estimate is an excellent bonus. 
 - The number of evaluations is quite high. 
...
The function below runs $O$(1000) $\vec x$ cases. `quadgk` is remarkably robust and efficient, but our free surface green's function will be **$O$(10000) times** more expensive than the source green function if we use it.
...
# ╔═╡ b08cea31-8dd2-43bb-9a5f-59458b87fa75
x_cases = reduce(vcat,(-R*cos(atan(a)),R*sin(atan(a)),-z) for R in 0:0.25:8, a in (0.,0.1,0.3,1/sqrt(8),0.5,1.0), z in 0.5 .^(0:8))

# ╔═╡ 421f9510-b96f-430f-893c-4393b5ee31d8
@time N_cases = [2/π*quadgk(T->Ni(x,y,z,T),-1,0,1)[1] for (x,y,z) in x_cases];

# ╔═╡ b0f89455-deae-4c86-8d3b-56497b04fe63
@time W_cases = [4quadgk(T->Wi(x,y,z,T),-Inf,Inf)[1] for (x,y,z) in x_cases];

# ╔═╡ 3fd01794-b62c-41ec-942d-6a1bb464f987
@time G_cases = [1/hypot(x,y,z) for (x,y,z) in x_cases];
...
|:---:|
|Error in a surrogate model when predicting previously "unseen" cases.|

As we have said many times in this class, you need to both understand your problem and understand your model options in order to make a good choice. 
 - Because $W(\vec x)$ is highly oscillatory (it's responsible for all the waves in the wake!) developing an accurate surrogate will be very tricky. Let's focus on $N(\vec x)$ instead since it is very smooth and symmetric. 
 - There are only three inputs $x,y,z$ which means a tool like deep-learning is completely overkill. Let's avoid all the issues training such a method and focus on polynomial interpolation methods.
 - We can sample the data at any $\vec x$ values we want using `quadgk`. Therefore we should use Chebyshev polynomials which (akin to Gauss-quadrature) have the smallest possible generalization error due to being sampled at [special points](https://en.wikipedia.org/wiki/Chebyshev_nodes).
...
begin 
	using Statistics
	maximum(abs,N_cases-nearfield_cases),std(N_cases-nearfield_cases)
end

# ╔═╡ 8d9f4e35-c032-4de1-87a4-620103f6c1bd
begin
	using NeumannKelvin:wavelike
	# Total over all x_cases
	@time wavelike_cases = [wavelike(x,y,z) for (x,y,z) in x_cases]
	# Challenging example
	@time wavelike(-10.,-1.,-1e-3)
	@time quadgk(T->Wi(-10.,-1.,-1e-3,T),-Inf,Inf)
	# The limit!
	@time wavelike(-10.,-1.,-0.)
	# @time quadgk(T->Wi(-10.,-1.,-0.,T),-Inf,Inf) # uncomment if you enjoy waiting
end; maximum(abs,W_cases-wavelike_cases),std(W_cases-wavelike_cases)

# ╔═╡ 0a9c0799-ad81-400e-adb1-9e46d1a1328c
md"
Let's interpret the results above:

 - The first time we use the function `nearfield` it compiles the code, generates about 10000 data points with `quadgk`, and fits the coefficients. This takes a while, but it's **much faster and easier** than dealing with a deep network surrogate.
 - Every function evaluation after the first one is crazy fast! `nearfield` is **~1000 times faster** than using `quadgk`!!
 - The error is tiny! The max error is 0.04%, the root mean square error is 0.01%.

## Activity 
 - How many panels would you need to break even between using `quadgk` and `nearfield` the first time?
 - How much faster would it be after the first time (in seconds)?
...
md"""
That takes care of $N$, but we need to accelerate $W$ as well...

### 3a. Stationary Phase

The integrand $W_i = \exp[z(1+T^2)]\sin[ψ(T)]=\exp[z(1+T^2)+iψ(T)]$ eventually goes to zero as $T\rightarrow \pm\infty$ since $z$ is negative. However, this drop off happens slower and slower as $z\rightarrow 0$, meaning there are more and more waves inside the envelop that we need to sample to estimate the integral accurately. 
...
md"""
### 3b. Path of Stationary Phase

The stationary points are important because the function oscillates slowly in those regions... What if we could change how we look at the function to maintain a stationary phase _everywhere_? 
...
$W_i = \exp[z(1+h^2)+i\psi(h)] = \exp[z(1+h^2)]\exp[-p^2]\sin(\psi(T_0))$

Since the phase $\psi(T_0)$ is constant, this doesn't oscillate. And since $e^{-p^2}$ decays super fast, we can evaluate the integral even when $z=0$ with just a few quadrature points!
...
$h = \pm\sqrt{(\psi(T_0)+i p^2)^2/x^2-1}$

As you can see in the plot below, this path **completely** removes the oscillations!!
...
md"""
### 3c. *Numerical* Stationary Phase

The example above is great, but we can't solve for the path analytically for $y\ne 0$. It's also a problem to use this approach near a stationary point when the integral **isn't** highly oscillatory (say when $z\le-R$). It's also unclear what to do when the two stationary points start to merge as $y/x\rightarrow\sqrt{1/8}$, or when the point disappears for $y/x>\sqrt{1/8}$!

The solution developed by Gibbs & Huybrechs in 2024(!!) is to define a range around each stationary point on the real line where the integral oscillates a small finite amount (say one oscillation) and then use a complex path of stationary phase from there to infinity. In `NeumannKelvin.jl` I use the `Roots.jl` package to find both the finite range and the complex path.

The code blocks below demonstrate the resulting `wavelike` function:
 - It is 100 times faster than `quadgk` over `x_cases`, and this relative speed up rapid increases as $z\rightarrow -0$.
 - It's super accurate and requires the same omputational cost across all the $x,y,z$ values!
"""
...
That was certainly a lot of work just to evaluate the Green's function for a single source! We deserve a demo!

Below, I have put the `nearfield` and `wavelike` functions together with the rankine source-sink term to plot the potential and wave elevation directly behind a moving unit-strength source. 

In case you think that was **too much work**, I've added a toggle so you can switch over to just use a 15 point Gauss quarature for the integration. What's the worst that could happen?
"""

# ╔═╡ fa8fc9ab-28a1-4480-b8b7-2c9c22f0325f
begin
	check = @bind real_only CheckBox(default=false)
	Fns = @bind Fn Slider(0.2:0.1:2,default=1,show_value=true)
	md"""Fn $Fns,  z $zs,  No ML or complex paths $check"""
end

# ╔═╡ 6c4464ae-baa6-4dbf-8619-556551da306f
begin
	function ϕ(x) 
		vecx = [x/Fn^2,0.,z/Fn^2]
		!real_only && return (nearfield(vecx...)+wavelike(vecx...))/Fn^2
		(2/pi*quadgk(T->Ni(vecx...,T),-1,1,maxevals=15)[1]+
		 4quadgk(T->Wi(vecx...,T),-Inf,Inf,maxevals=15)[1])/Fn^2
	end
	using ForwardDiff:derivative
	x_rng = range(-30,10,1000)
	ϕ_rng = ϕ.(x_rng)
	ζ_rng = Fn^2 .* derivative.(ϕ,x_rng)
	flow = let
		plt1 = plot(x_rng,ϕ_rng,label=nothing,xlabel="",ylabel="ϕ(z=y=0)")
		plt2 = plot(x_rng,ζ_rng,label=nothing,title="",xlabel="x",ylabel="ζ(y=0)")
		plot(plt1,plt2,layout=(2,1),title="source at [0,0,$z], Fn=$Fn")
	end
end
...
manifest_format = "2.0"
project_hash = "f61344eeab5e792b788463f880e288abdbb202fc"
