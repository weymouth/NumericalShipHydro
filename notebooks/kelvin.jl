...
$N = \frac 1{|\vec x|}+\frac 2\pi\int_{-\infty}^\infty N_i(\vec x,T) dT,\quad W = 4 H(-x)\int_{x/|y|}^\infty W_i(\vec x,T) dT$
...
This comes up in wave analysis all the time. Here's a [physics video](https://www.youtube.com/watch?v=-UgQEHHXTRM) where stationary phase is used to derive the group velocity of a wave. 
