...
where $\ell\equiv\frac{U^2}g$ is the Kelvin length and $\vec x = (\vec\xi-\vec a')/\ell$ is the Kelvin-scaled vector from the image sink. The near-field and wave-like contributions are defined by a pair of integrals:
...
### Activity
 - How should the Kelvin length $\ell=\frac{U^2}g$ relate to the wavelength? Do the results match the theory when you change $\ell$?
