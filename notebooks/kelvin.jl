...
The plot above is the potential and wave field generated by a source moving in the positive x direction under the water. The steady potential at point $\vec \xi$ due to a unit-strength source at point $\vec a$ is the sum of three contributions:
...
x_cases = reduce(vcat,(-R*cos(atan(a)),R*sin(atan(a)),-z) 
...
$W_i(h(p)) = \Im\exp(z(1+h^2)+i\psi(h)) = \exp(z(1+h^2)-p^2)\sin(\psi(T_0))$
...
let
	...
	# Plot vs p
	plt2 = plot(range(-3,3,1000),p->Wi(h(p)),xlabel="p",
		ylabel="Wᵢ(h(p))",label="")
