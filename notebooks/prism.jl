...
md"""
The plot above shows the computed $c_p$ value on the elliptical prism. There is a stagnation pressure on the leading edge, but we can see low pressure regions along the hull which vary in location and magnitude with the $\text{Fn}$.

We can further explain and investigate this behaviour using the free surface elevation $\zeta$, shown in the plots below. The deepwater dispersion relationship gives $g\lambda/U^2 = 2\pi$, meaning $\lambda/l = 2\pi \text{Fn}^2$. Therefore we should have $\lambda(\text{Fn}=0.57)\approx 2l$ at the high speed and $\lambda(\text{Fn}=0.2)\approx l/4$ at the low speed, and we see this behaviour verified in our waterline predictions. 
...
Fn_sweep = map(logrange(0.2,0.6,40)) do Fn
	q = influence(panels;G,Fn)\first.(panels.n)
	(Fn=Fn,Cw=steady_force(q,panels;G,Fn)[1])
end|> Table;
