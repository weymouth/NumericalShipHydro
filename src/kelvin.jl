...
function kelvin(ξ,α;Fn=1,kwargs...)
    ...
    T₀ = ifelse(y^2<x^2,0,x/y) # integration center
    N = 1/hypot(x,y,z)+2/π*quadgl_inf(T->Ni(x,y,z,T-T₀);kwargs...)
...
using Plots
xgl,wgl = gausslegendre(61);
plt = plot(ylims=(-3,2))
for y = [0,0.1,1,10]
    plot!(plt,-10:0.01:3,x->derivative(x->kelvin([x,y,0],[0,0,-1];Fn=0.6,xgl,wgl),x),label=y)
end
display(plt)

x,y,z,Fn = -.5,.1,0,0.6
xgl,wgl = gausslegendre(21);
derivative(x->kelvin([x,y,0],[0,0,-1];Fn=0.6,xgl,wgl),x) # should be ~ -1

x,y,z = -1.3888,2.777,-2.777
T₀ = x/hypot(z,y) # integration center
plot(-10:0.1:10,T->Ni(x,y,z,T-T₀))
...
source_elevation_plot()

plt = plot(aspect_ratio=:equal)
@gif for Fn = 1:-0.05:0.2
    ζ(x,y) = derivative(x->kelvin([x,y,0],[0,0,-1];Fn),x) # unit-point source
    x,y = -15:0.1:3,-6:0.1:6; z = ζ.(x',y)/2.5
    contour(x,y,z,levels=filter(≠(0),-0.9:0.15:0.9),title="Point-source wave elevation, Fn=$Fn",legend=false)
end
